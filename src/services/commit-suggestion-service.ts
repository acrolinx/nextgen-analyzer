...
export async function createPRCommitSuggestions(
  ...
  try {
    ...
    if (validSuggestions.length === 0) {
      core.info('No suggestions for files changed in this PR')
      return
    }

    core.info(
      `Found ${validSuggestions.length} suggestions for ${prFiles.length} changed files`
    )

    // Create a single pending review with all suggestions
    // For suggestions, we need to use the correct format
    const comments = validSuggestions.map((suggestion) => ({
